    ...
    {loading ? <p role="status">Opening your item…</p> : <>
      ...
      {(!contentId || details) && <>
        {!draft.contentType ? <section className="studio-type-choice"><h2>What do you want to create?</h2><div>{[['quiz','Quiz','Interactive questions for learning and practice.'],['assessment','Assessment','Check knowledge and understanding.']].map(([value,label,description]) => <button key={value} disabled={!editable} onClick={() => changeDraft({ contentType: value })}><span aria-hidden="true">{value === 'quiz' ? '✦' : '✓'}</span><h3>{label}</h3><p>{description}</p><strong>Start {label.toLowerCase()} →</strong></button>)}</div></section> : <>
          ...
          {step === 3 && <section className="studio-editor-panel"><div className="studio-section-heading"><div><h2>{details?.content.status === 'draft' ? 'One last look' : typeLabel(draft.contentType) + ' details'}</h2><p>{details?.content.status === 'draft' ? 'Review the saved version before it reaches your learners' : 'This item is read-only'}</p></div><Button variant="secondary" disabled={!review || busy} onClick={() => setPreview(review)}>Preview</Button></div>{review ? <StudioReview details={review} /> : <Button onClick={() => void openReview()} disabled={blocked || dirty}>Load latest review</Button>}<div className="studio-savebar">{supported && review?.capabilities.can_publish && <PulseSpectralButton variant="pulse" disabled={blocked || dirty || !review.questions.length} onClick={() => setConfirmation('publish')}>Publish</PulseSpectralButton>}{supported && review?.capabilities.can_archive && <Button variant="secondary" disabled={blocked} onClick={() => setConfirmation('archive')}>Archive</Button>}</div></section>}
        </>}
      </>}
    </>}
