...
function configuredDestination() {
  return {
    approvedProjectRef: clean(import.meta.env?.VITE_GO_PRACTICE_PROJECT_REF),
    approvedOrigin: clean(import.meta.env?.VITE_GO_PRACTICE_ORIGIN),
  }
}

export function resolveGoPracticeDestination(destination, {
...
export function assertGoPracticeDestination(destination, options) {
  const result = resolveGoPracticeDestination(destination, options)
  if (!result.allowed) throw new Error('GO Practice is not enabled for this destination.')
  return result
}
