...
function configuredDestination() {
  return {
    approvedProjectRef: clean(import.meta.env?.VITE_GO_HOSTED_PROJECT_REF),
    approvedOrigin: clean(import.meta.env?.VITE_GO_HOSTED_ORIGIN),
  }
}
...
export function assertGoHostedDestination(destination, options) {
  const result = resolveGoHostedDestination(destination, options)
  if (!result.allowed) throw new Error('GO Hosted Sessions are not enabled for this destination.')
  return result
}
