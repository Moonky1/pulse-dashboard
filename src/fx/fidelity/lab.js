...
<header><span>PULSE / FX-1B</span><span>LOCAL MATERIAL STUDY</span></header>
...
<section class="tuning"><h2>Material calibration</h2><p>Adjust candidate B. A and C stay at their calibrated settings.</p><div id="controls"></div><button id="reset">Reset B</button></section>
...
<footer>FX-1B · local only · no product routes or backend connections</footer>`
...
function buildControls(){document.querySelector('#controls').innerHTML=controls.map(([key,min,max,step])=>`<label>${key}<output id="value-${key}">${tuning[key].toFixed(2)}</output><input aria-label="${key}" data-control="${key}" type="range" min="${min}" max="${max}" step="${step}" value="${tuning[key]}"></label>`).join('');document.querySelectorAll('[data-control]').forEach(input=>input.addEventListener('input',()=>{tuning[input.dataset.control]=Number(input.value);document.querySelector(`#value-${input.dataset.control}`).textContent=Number(input.value).toFixed(2);renderers.B.setConfig(tuning)}))}
buildControls()
document.querySelector('#reset').addEventListener('click',()=>{tuning={...PRESETS.B};renderers.B.setConfig(tuning);buildControls()})
...
async function filmstrip(id) {
  ...
  if(id===3){
    const updateFrame=()=>{const canvas=document.querySelector('#reference-frame');canvas.getContext('2d').drawImage(player,0,350,1080,382,0,0,960,340);document.querySelector('#reference-caption').textContent=`Reference frame ${player.currentTime.toFixed(2)}s / ${player.duration.toFixed(2)}s`}
