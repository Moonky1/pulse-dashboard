...
export function createMaterial(canvas, config, { shape = 0, onStatus = () => {} } = {}) {
  ...
  const names = ['resolution','pointer','time','interaction','press','shape','dispersion','bend','thickness','fresnel','specular','caustic','noiseScale','speed','pointerForce','absorption','balance','exposure']
  let uniforms = {}
  ...
  const lost=e=>{e.preventDefault();error='Context lost';canvas.dataset.renderer='fallback';clear();onStatus('Context lost — CSS fallback')}
  const restored=()=>{initialize();schedule(true)}
  const observer=new IntersectionObserver(([entry])=>{visible=entry.isIntersecting;clear();if(visible){if(!gl)initialize();previous=performance.now();schedule(true)}},{rootMargin:'40px'})
