...
import { listStaffInvitations, loadStaffInvitationOptions, normalizeStaffInvitationOptions, reinviteStaffInvitation, resendStaffInvitation, revokeStaffInvitation, sendStaffInvitation } from './adminApi.js'
...
test('invitation options normalize only authoritative UUID catalogs and exact scopes', () => {
  const result = normalizeStaffInvitationOptions({
    ...
    campaigns: [{ id: CAMPAIGN, code: 'campaign', name: 'Campaign' }],
    operating_units: [{ id: UNIT, campaign_id: CAMPAIGN, code: 'unit', name: 'Unit' }],
    teams: [{ id: ID, campaign_id: CAMPAIGN, operating_unit_id: UNIT, code: 'team', name: 'Team' }],
    positions: [],
  ...
  assert.equal(result.teams[0].campaignId, CAMPAIGN)
  assert.equal(result.operatingUnits[0].campaignId, CAMPAIGN)
...
test('send, re-invite, resend, and revoke invoke only the trusted Edge boundary with server timestamps', async () => {
  const calls = []
  ...
  await reinviteStaffInvitation(client, { id: ID }, { email: 'person@example.test', departmentId: DEPARTMENT, roleId: ROLE, scopeType: 'global' })
  ...
  assert.deepEqual(calls.map(([name, body]) => [name, body.action]), [
    ['pulse-staff-invitations', 'send'], ['pulse-staff-invitations', 'send'], ['pulse-staff-invitations', 'resend'], ['pulse-staff-invitations', 'revoke'],
  ])
  assert.match(calls[0][1].requestKey, /^[0-9a-f-]{36}$/i)
  assert.equal(calls[1][1].previousInvitationId, ID)
  assert.equal(calls[2][1].expectedUpdatedAt, '2026-09-10T00:00:00Z')
  assert.equal(calls[3][1].expectedUpdatedAt, '2026-09-10T00:00:00Z')
})
