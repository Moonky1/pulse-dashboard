...
test('People and Staff Tree are shared protected directory surfaces', async () => {
  ...
  assert.match(tree, /buildStaffTreeV2/)
  assert.match(tree, /Find a person or team/)
  assert.match(tree, /Reporting lines appear only when they are explicitly recorded/)
  assert.match(tree, /Quality coverage is shown as secondary context/)
  assert.doesNotMatch(tree, /STAFF_TREE_PROTOTYPE/)
