...
test('role scopes clearly distinguish global, department, Campaign, and team access', () => {
  ...
  assert.equal(roleScopeLabel({ scopeType: 'global' }, directory), 'All Pulse')
  assert.equal(roleScopeLabel({ scopeType: 'department', departmentId: 'd1' }, directory), 'Corporate')
  assert.equal(roleScopeLabel({ scopeType: 'campaign', campaignName: 'Garrett' }, directory), 'Garrett')
  assert.equal(roleScopeLabel({ scopeType: 'team', teamId: 't1' }, directory), 'North')
})
