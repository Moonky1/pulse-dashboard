            ...
            <div className="admin-table admin-people-table" role="table">
              ...
              {filtered.map((user) => (
                <Link className="admin-user-row" role="row" key={user.id} to={user.status === 'pending_approval' ? `/admin/pending/${user.id}` : `/admin/users/${user.id}`} aria-label={`Open ${user.fullName}'s staff profile`}>
                  <div className="admin-user-identity admin-user-identity--avatar"><StaffAvatar name={user.fullName} customAvatarPath={user.customAvatarPath} googleAvatarUrl={user.googleAvatarUrl} avatarUpdatedAt={user.avatarUpdatedAt} /><span><strong>{user.fullName}</strong><small>{user.employeeId || 'Employee ID pending'}</small></span></div>
