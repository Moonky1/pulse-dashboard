  ...
  return (
    <main className="admin-content">
      ...
      <div className="admin-detail-grid">
        <Card level={2} className="admin-detail-card admin-detail-card--identity"><p className="admin-section-label">Identity</p><h2>Contact and identity</h2><dl>{user.displayName && <Detail label="Display name">{user.displayName}</Detail>}<Detail label="Employee ID">{user.employeeId}</Detail><Detail label="Email">{user.email}</Detail><Detail label="Joined Pulse">{formatPulseDate(user.pulseJoinedOn)}</Detail></dl><JoinedPulseAdministration user={user} allowed={!pendingOnly && canManageUsers(permissionKeys)} onChanged={refresh} /></Card>
