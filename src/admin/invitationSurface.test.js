...
test('Staff invitation email has a branded, truthful and portable HTML template', async () => {
  ...
  assert.match(template, /This personal invitation expires in 72 hours/)
  assert.match(template, /Pulse will never ask you to share this link or your password/)
  assert.match(template, /If you weren’t expecting this invitation, you can safely ignore it/)
  assert.match(template, /max-width:560px/)
  assert.match(template, /@media only screen and \(max-width: 600px\)/)
  assert.match(template, /pulse-action-cell/)
  assert.match(template, /Pulse · Kampaign Kings/)
  assert.match(template, /\{\{ \.ConfirmationURL \}\}/)
  assert.equal(template.match(/\{\{ \.ConfirmationURL \}\}/g)?.length, 1)
  assert.doesNotMatch(template, /script|service_role|access token|https?:\/\//i)
