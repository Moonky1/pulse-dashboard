    ...
    <main className="auth-workspace">
      ...
      <section className="auth-workspace-card">
        <div className="auth-workspace-intro">
          ...
          <h1>Good to see you, {profile?.display_name || profile?.full_name}.</h1>
        ...
        <div className="auth-workspace-actions">
          ...
          {studioAccess.state === 'allowed' && <Link className="auth-workspace-destination" to="/studio"><span className="auth-workspace-symbol" aria-hidden="true">✦</span><span><strong>Studio</strong><small>{canCreateStudioContent(studioAccess.permissionKeys) ? 'Create and manage training' : 'Browse training content'}</small></span><b aria-hidden="true">→</b></Link>}
          {adminAccess.state === 'allowed' && <Link className="auth-workspace-destination" to="/admin/users"><span className="auth-workspace-symbol" aria-hidden="true">⌁</span><span><strong>Administration</strong><small>People, access, and organization</small></span><b aria-hidden="true">→</b></Link>}
        </div>
      </section>
    </main>
  )
