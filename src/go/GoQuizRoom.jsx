...
export default function GoQuizRoom() {
  ...
  const refreshAll = useCallback(async () => {
    ...
    setLoading(false)
  }, [fetchRoom, fetchPlayers, fetchAnswers])
  ...
  }, [code, nav, refreshAll, fetchPlayers, fetchAnswers])
  ...
  if (state === 'finished') {
    ...
    return (
      <div className="grm grm-finished">
          ...
          <section className="grm-final-section grm-top-performers">
            ...
            <div className="grm-top-list">
              {topPerformers.map((player) => {
                ...
                return (
                  <div key={player.id} className={`grm-top-row ${player.id === playerId ? 'me' : ''}`}>
                    <span className="grm-top-rank">
                      {visual ? (
                        <img src={visual.medal} alt="" />
                      ) : (
                        `#${player.rank}`
                      )}
                    </span>
        ...
        {isHost && needsReview.length > 0 && (
          <section className="grm-final-section grm-practice-section">
            <div className="grm-final-section-head">
              <div>
                <span>Coaching List</span>
                <h2>Needs Practice</h2>
              </div>

              <strong>{needsReview.length} flagged</strong>
            </div>

            <div className="grm-practice-grid">
              {needsReview.map((player) => {
                ...
                return (
                  <details key={player.id} className={`grm-practice-card ${practice.className}`}>
                    <summary>
                      <img src={practice.icon} alt="" />
                      <span>
                        <strong>{practice.label}</strong>
                        <em>{player.name} · {player.correctCount}/{gameQuestionCount} · {player.accuracy}%</em>
                      </span>
                    </summary>

                    <div className="grm-practice-misses">
                      {player.questionResults
                        .filter((answer) => !answer.correct)
                        .map((answer) => (
                          <div key={`${player.id}-miss-${answer.questionIndex}`} className="grm-practice-miss">
                            <b>Q{answer.questionIndex + 1}</b>
                            <span>{answer.question}</span>
                            <small>
                              Picked: {answer.noAnswer ? 'No answer' : `${answer.selectedLetter} · ${answer.selectedOption}`}
                            </small>
                          </div>
                        ))}
                    </div>
                  ...
                  <details key={player.id} className="grm-participant-card">
                    <summary>
                      ...
                      <span className="grm-participant-name">
                        ...
                        <small>
                          Rank #{player.rank} · {player.correctCount}/{gameQuestionCount} correct · {player.accuracy}%
                        </small>
                      </span>
                    ...
                    <div className="grm-agent-answer-sheet">
                      <div className="grm-agent-answer-head">
                        ...
                        <span>{player.correctCount}/{gameQuestionCount} correct</span>
                      </div>
                      ...
                      <div className="grm-agent-answer-grid">
                        {player.questionResults.map((answer) => (
                          <div
                            ...
                            className={`grm-agent-answer-card ${answer.correct ? 'correct' : answer.noAnswer ? 'no-answer' : 'wrong'}`}
                          >
                            <div className="grm-agent-answer-top">
                              <span>Q{answer.questionIndex + 1}</span>
                              <strong>{answer.correct ? 'Correct' : answer.noAnswer ? 'No Answer' : 'Wrong'}</strong>
                            </div>
                            ...
                            <div className="grm-agent-answer-lines">
                              ...
                              <div>
                                ...
                                <b>{answer.correctLetter} · {answer.correctOption}</b>
                              </div>
                            </div>
                          </div>
                        ))}
