...
select throws_ok($$insert into public.training_questions(content_id,position,question_type,prompt,answer_options,correct_answer) values ('4a000000-0000-4000-8000-000000000001',4,'multiple_choice','Invalid index','["A","B"]','3')$$,'22023',null,'multiple-choice correct index must reference an option');
select throws_ok($$insert into public.training_questions(content_id,position,question_type,prompt,answer_options,correct_answer) values ('4a000000-0000-4000-8000-000000000001',4,'true_false','Invalid options','["Yes","No"]','true')$$,'22023',null,'true/false does not accept arbitrary options');
select throws_ok($$insert into public.training_questions(content_id,position,question_type,prompt,answer_options,correct_answer) values ('4a000000-0000-4000-8000-000000000001',4,'text','Missing accepted answers','[]','[]')$$,'22023',null,'text questions require accepted answers');
